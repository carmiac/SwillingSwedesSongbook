...
def make_author_index(infile, outfile):
    ...
    with open(outfile, "w") as f:
        ...
        for author in sorted(authors, key=str.casefold):
            ...
            f.write(
                song_entry.format(songnum=songs[0]["songnum"], link=songs[0]["link"])
            )
...
def _make_book_dictionary(infile):
    books = {}
    altnames = {}  #
    # process infile:
    with open(infile, "r") as f:
...
def make_verse_index(infile, outfile):
    books = _make_book_dictionary(
        infile,
    )
    ...
    with open(outfile, "w") as f:
        for b in [x for x in books if len(books[x]) > 0]:
            ...
            for song in sorted(books[b], key=lambda k: int(k["verses"].split(":")[0])):
                f.write(
                    entry.format(
                        verse=song["verses"], songnum=song["songnum"], link=song["link"]
                    )
                )
            # write end block
            f.write(endsection.format())
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=["songs.sbd"],
        help="Song file(s) to include, wildcards supported. ( default %(default)s)",
    )
    parser.add_argument(
        "-d",
        "--header",
        default="",
        help="Header for the entire songbook (default %(default)s)",
    )
    parser.add_argument(
        "-t",
        "--tex",
        nargs="*",
        default=["*.tex"],
        help="tex file(s) to process, wildcards supported. (default %(default)s)",
    )
    parser.add_argument(
        "-p",
        "--pdflatex",
        default="pdflatex",
        help="command to call pdflatex (default %(default)s )",
    )
    parser.add_argument(
        "-o",
        "--outfile",
        default="songs.sbd",
        help="File of songs created from infile(s) and referenced in tex files. If infile = outfile, the header is not used. (default %(default)s)",
    )
    parser.add_argument(
        "-s",
        "--sort",
        default=True,
        help="sort the list of infiles (default %(default)s)",
    )
    parser.add_argument(
        "-c",
        "--clean",
        action="store_true",
        help="Clean the build and install directories (default %(default)s)",
    )
    args = parser.parse_args()

    # Clean build and output directories if requested
    if args.clean:
        try:
            shutil.rmtree("build")
    ...
    infiles = [
        item for sublist in [glob.glob(x) for x in args.infile] for item in sublist
    ]
    if infiles[0] != args.outfile:
        make_songfile(
            infiles, os.path.join("build", args.outfile), args.header, args.sort
        )

    # process all tex files to generate indexed pdfs
    texfiles = [
        item for sublist in [glob.glob(x) for x in args.tex] for item in sublist
    ]

    # Copy text files to build directory
    for tex in texfiles:
        shutil.copy(tex, "build")
    ...
    # Copy all of the .txt files to the build directory
    for txt in glob.glob("*.txt"):
        shutil.copy(txt, "build")
